-- public.glu_dt_v2 исходный текст

CREATE OR REPLACE VIEW public.glu_dt_v2
AS WITH success_acts AS (
         SELECT erv.user_id,
            erv.activity_id,
         ...
         SELECT dt.k_day,
            ...
            dt.m2_progress
           FROM dt
          WHERE dt.k_day < '2024-03-01'::date AND (dt.user_id IN ( SELECT DISTINCT dt_1.user_id
                   FROM dt dt_1
                  WHERE dt_1.k_day = '2024-03-01'::date AND abs(dt_1.current_progress - dt_1.m2_progress) <= 10)) AND (dt.user_id IN ( SELECT uv.user_id
                   FROM users_v2 uv
