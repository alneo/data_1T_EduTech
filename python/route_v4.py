...
if len(sys.argv) == 1:
    print("Используй:________________________________________python3 route_v4.py <model_name> <user_id> <week_number|date_set>")
    print("Пример пользователь и текущее состояние:__________python3 route_v4.py project_1911 19489")
    print("Пример пользователь и 3 неделя:___________________python3 route_v4.py project_1911 19489 3")
    print("Пример пользователь и получить на указанную дату:_python3 route_v4.py project_1911 19489 2023-12-14")
    print("Модели : project_1611(m2_progress) project_1911(m2_success) project_2411_clf(m2_success) project_2911(m2_success)")
    sys.exit(1)
...
file_SQL = 'sql/sql_oneuser_data_v4.sql'
dirname_saved_models = 'saved_models'

if model_info == 'project_2411_clf':
    joblib_name = 'gb_model'
    metrika = 'm2_success'
    file_SQL = 'sql/sql_oneuser_data_v4_success.sql'

if model_info == 'project_2911':
    joblib_name = 'gb_model'
    metrika = 'm2_success'
    dirname_saved_models = 'saved_clf_models'
    file_SQL = 'sql/sql_oneuser_data_v4_success.sql'
...
def get_sql_content_with_replacements(user_id, start_date):
    ...
    try:
        ...
        logs.append(f"Файл '{file_SQL}' не найден.")
    except Exception as e:
        logs.append(f"Ошибка: {e}")
...
def predict_for_user(week, user_id, date_cur):
    ...
    try:
        ...
        if data_start is None:
            #logs.append("Не определили дату начала курса для пользователя")
            logs.append("Not get date_start for user")
        ...
        model = joblib.load(f'{PATH}/{dirname_saved_models}/{joblib_name}_week_{week}.joblib')

        # Инициализируем переменную для хранения данных
        sql, logs = get_sql_content_with_replacements(user_id, "'"+data_start[1]+"'")
        if len(logs)==0:
        ...
        logs.append(f"Date start is: {data_start[1]}.")
        ...
        if error == 0:
            ...
            if metrika not in user_data.columns:
                logs.append(f"Column '{metrika}' is missing from the dataset.")
                error = 1

            if error == 0:
                # Получаем реальное значение целевого признака
                actual_value = user_data[metrika].values[0]
                logs.append(f"Actual value of '{metrika}' for user_id {user_id}: {actual_value:.2f}")

                start_time = time.time()
                # Подготовка данных для предсказания
                X_user = user_data.drop([metrika], axis=1)
                ...
                if error == 0:
                    ...
                    if model_info == 'project_2411_clf' or model_info == 'project_2911':
                    ...
                    logs.append(f"Predicted value of '{metrika}' for user_id {user_id} in week {week}: {result:.2f}")
