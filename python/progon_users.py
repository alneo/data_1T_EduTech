...
def postgresql_model_stat_check(model_info, user_id, day_num):
    ...
    try:
        connection = psycopg2.connect(host=host, port=port, database=database, user=user, password=password)
        cursor = connection.cursor()
        sql = "select id from model_stats where model_info=\'"+model_info+"\' AND id_user="+str(user_id)+" AND day_num="+str(day_num)+" limit 1;"
        cursor.execute(sql)
        data = cursor.fetchone()
        cursor.close()
        connection.close()
...
def postgresql_users_all_query():
    ...
    try:
        connection = psycopg2.connect(host=host, port=port, database=database, user=user, password=password)
        cursor = connection.cursor()
        sql = "select user_id from users_v2 GROUP BY user_id ORDER by user_id ASC;"
        cursor.execute(sql)
...
if len(sys.argv) < 2:
    print("Используй:________________________________________python3 progon_users.py <model_info> <user_id>")
    print("Просчет пользователя по модели от начала обучения до конца 10 недели и сохранения результата")
    print("python3 progon_users.py project_1911 19254 - модель project_1911 просчитать одного пользователя 19254")
    print("python3 progon_users.py project_1911 0 - модель project_1911 просчитать всех пользователей")
    sys.exit(1)
...
for user_id in users:
    ...
    if data_start != None:
        ...
        for i in range(1, 70):
            date_obj = datetime.strptime(data_start[0], '%Y-%m-%d')
            ...
            if rez == None:#Нет такой записи
                command = f"python3 route.py {model_info} {user_id} {date_cur}"
