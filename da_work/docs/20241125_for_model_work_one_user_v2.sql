--это код для вебки. При запуске запрашиваются переменные 
--:user_id, :start_date, current_date
--вводишь номер и получаешь датасет для входа в модель, состоящий из одной строки - информации по одному юзеру
--конечную дату везде установила current_date. Начальную нужно задать так же как и пользователя (через переменную). Замечание! Задаем в формате '2023-12-01' обязательно с кавычками
--если ввести некорректные данные даты, то в полях результирующей таблицы будут NULL
...
from dt where k_day < current_date
				and user_id in(select distinct user_id from dt where k_day = current_date and abs(current_progress - m2_progress) <= 10)
