--переменные: :start_date - дата начала курса, :user_id - уникальный идентификатор пользователя
...
from generate_series
        (:start_date::timestamp  --'2024-06-01'::timestamp 
        , current_date --'2024-09-15'::timestamp
        , '1 day'::interval) dd 
)
,
metrics_1 as
...
,
dt_final as(
select k_day, user_id, course_id, required_activities_delay, success_required_done, success_optional_done, mean_result_required, cur_date_progress, current_progress
	, case 
	...
	, case 
		when course_id = 76 then case  --курс 76 - девопсы. аттестация от 10 баллов
							when course_attestation >= 10 then 1 else 0
							end
		when course_id in(77, 82, 83) then case  --все остальные курсы аттестация от 60 баллов
